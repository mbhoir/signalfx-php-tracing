...
static int dd_fatal_error_to_meta(zend_array *meta, dd_error_info error) {
    if (error.type) {
        ...
        zend_symtable_str_update(meta, ZEND_STRL("sfx.error.kind"), &tmp);
    }

    if (error.msg) {
        zval tmp = ddtrace_zval_zstr(zend_string_copy(error.msg));
        zend_symtable_str_update(meta, ZEND_STRL("sfx.error.message"), &tmp);
    }

    if (error.stack) {
        zval tmp = ddtrace_zval_zstr(zend_string_copy(error.stack));
        zend_symtable_str_update(meta, ZEND_STRL("sfx.error.stack"), &tmp);
    }
